...
(psx.vram_viewer.vram_index_image) is flat grey index values rather
...
area's chunk contributes on top (see psx/vram_map). A texel two
...
# formats/archive/format_detect.py's own MDAT detector). The order is what a
...
class Patch:
    ...
    shaped to match what formats.sprites.sprt_render.VRAMTextures.piece_image
    ...
    def dest(self):
        ...
        4096x512 texel atlas psx.vram_viewer's images use."""
...
def regions_from_bgmp(bgmp):
    ...
    stacked below it (see formats/background/bgmp_parser.py). One box per palette
    ...
    the wrong colours."""
    from formats.background.bgmp_parser import PALETTE_STRIDE, TILE
...
def area_regions(idx_path, dat_path, chunk_index):
    ...
    from formats.geometry.mdat import area_mdat_entries, exportMDAT
...
def render(vram_bytes, regions):
    ...
    straight from the same numpy array psx.vram_viewer.vram_index_image
    would turn into one, so a caller can composite further before
    handing it to Qt - which psx/vram_viewer.py's own wrapper does."""
    import numpy as np
    from PIL import Image

    from psx.vram_viewer import TEXEL_HEIGHT, TEXEL_WIDTH, vram_texels
...
def render_layer(vram_bytes, regions, cells=True):
    ...
    from formats.sprites.sprt_render import VRAMTextures
    from psx.vram_viewer import TEXEL_HEIGHT, TEXEL_WIDTH
