"""
Reverse of formats/text/txtd.py's preview() function.
...
from formats.text.tombadict import letters as LETTERS
...
def refresh_tables(prefer=None):
    ...
    that import has to reach it too (see formats/text/translation.py)."""
...
def pointer_shift():
    ...
    16-bit units - see formats/text/jptext. The master table is not affected:
...
def encode_text(text, cells=False):
    ...
    formats/text/jptext). It means nothing on a Latin build, where both
