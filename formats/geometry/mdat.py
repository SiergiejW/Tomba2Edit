...
from game.texture_window import QUAD
...
def find_area_mdat_location(idx_path, chunk_index):
    ...
    and the same scan formats.collision.scld_parser.find_area_scld_location does
    ...
    # Slot 8 on every build but the demos (game/game_build.py).
...
def area_mdat_entries(idx_path, dat_path, chunk_index):
    ...
    test formats.archive.format_detect uses - 0xFFFF at +4, which is the
...
def exportMDAT(drwa_addr, datpath):
    ...
    model_data = {
        ...
        # an area's cell drawer reads as flags - game/texture_window.py.
        'face_flags': [],
        'tri_count': 0,
        'quad_count': 0,
        ...
        # formats/drawmaps/drwa_parser.py for the layout these come from.
    ...
    def adjust_uv(raw_u, raw_v, page):
        ...
        # psx.vram.atlas_uv, which aims at the middle of the
    ...
    with open(datpath, "rb") as rom:
        ...
        # second - see formats/drawmaps/drwa_parser.py.
        ...
        while eye < drwa_size:
            ...
            if 0 < val < 0xFFFF:
                ...
                for slot in range(num_tris):
                    ...
                    # formats/models/smst_parser.py on why they matter.
