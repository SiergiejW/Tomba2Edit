...
formats/geometry/mdat_viewer.py, because the polygons are the same polygons.
...
every part inside every other one (see formats/models/smst_parser.py). The
...
from formats.archive.format_detect import FormatError
...
from gui.widgets import export_dialog, polygon_pick
from gui.widgets.texture_panel import TexturePanel
from formats.models import smst_edit
from game import labels
from game import placement
from formats.models.smst_parser import parse_smst, read_smst_bytes
...
# is drawn at 1000 (formats/collision/scld_render.UNIT_SCALE); a character is
...
# The selection outline, matching formats/geometry/mdat_viewer.py: yellow for the
...
# formats/geometry/mdat_panel.py uses, for the same bits.
...
class SMSTViewer(ClutAnimationMixin, CameraEventMixin, QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        # drawn with - see gui/widgets/polygon_pick.py, which MDAT shares.
        ...
        # see set_pose and formats/animation/anmp_viewer.py.
        self.pose = None
        ...
        # The world origin, drawn over the model - see gui/widgets/origin_axes.py.
        self.show_origin = False
        self.origin_axes = OriginAxes()

        # Animated textures - see formats/animation/clut_animation.py. An asset pack
        ...
        # as the view draws it, one game frame a tick (gui/widgets/view_gif.py).
    ...
    def load_smst_data(self, dat_file_path, address, size):
        ...
        try:
            ...
            # once kept whole for copy/paste (see formats/models/smst_edit.py)
    ...
    def _model_name(self):
        ...
        formats/models/smst_viewer.SMSTPanel._name_of, which does the same
    ...
    def set_pose(self, transforms, pivots):
        ...
        the point each group turns about - see formats/animation/anmp_skeleton.py.
    ...
    def _clut_from_bytes(raw, transparent=False):
        ...
        VRAM (see formats/animation/clut_anim.py) and have to be read the same
        way, STP bit and all."""
        clut = []
        for i in range(16):
            at = i * 2
    ...
    def frame_model(self, heading=MODEL_HEADING, pitch=MODEL_PITCH):
        ...
        this finds (see gui/widgets/camera_controls.py)."""
    ...
    def initializeGL(self):
        ...
        if not self.shader_program.addShaderFromSourceCode(
                ...
                // psx/draw_order.py; DEPTH_TIE is its step.
...
class SMSTPanel(QWidget):
    ...
    def __init__(self, viewer: SMSTViewer, parent=None):
        ...
        self.table.setToolTip(
            ...
            "game/placement.py. The Level Editor reads the same "
            "names.")
