...
def rest_pose(model, hierarchy):
    ...
    and those joints are measured - see formats/animation/tomba_rest.py - rather
    than guessed at from bounding boxes the way rest_pivots() below has
    to for everything else."""
    from formats.animation.tomba_rest import REST_POSE
...
def _euler_matrix(rx, ry, rz):
    ...
    f_UpdateActorSequentialAxisPartTransforms, and formats/animation/skeleton_tree.py
