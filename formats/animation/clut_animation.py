...
Two different things move, and this plays both:
  ...
  PALETTES  the game swaps a fresh 16-colour CLUT into VRAM each frame
            ...
            read by formats/animation/clut_anim.py.
...
# code formats/animation/clut_anim.py has not followed, so nothing on the disc
...
def prepare_animation_data(vram, model, overlay_path, uv_models=None):
    ...
    in) and nothing in it sits in one cell - see formats/animation/uv_anim.py."""
    cluts, uvs, rules = {}, {}, ()
    found = []
    if overlay_path:
        try:
...
class ClutAnimationMixin:
    ...
    def init_clut_animation(self):
        ...
        # Texture windows (game/texture_window.py): the rules this
