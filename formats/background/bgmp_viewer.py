...
cells (see formats/background/bgmp_parser.py), so the two views are wired
...
from formats.background.bgmp_parser import BGMPError, PAGE_TILES, TILE, load_bgmp
from formats.background.bgmp_render import (
...
class BGMPViewer(QWidget):
    ...
    def load_bgmp_data(self, dat_file_path, dat_start, offset, size,
        ...
        psx.vram_viewer.decode_vram_bytes), which holds both the tiles
