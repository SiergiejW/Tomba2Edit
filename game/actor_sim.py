...
game/actor_assembly.py reads one prop's init by hand. This does
...
lifecycle routine for a few frames in psx/cpu.py. Whatever the
...
game/decomp_symbols.py from the US decomp:
...
from psx import cpu as psx_cpu
from psx.cpu import CPU, PASS, EmuError, s16, s32  # noqa: F401 - s16 used by level_scene
...
# while it is open (game/game_build.py).
...
class World:
    ...
    def _clip_poses(self, actor, budget):
        ...
        from formats.animation.sequences import SequenceError, read_clip
        read = self.mem.read
    ...
    def _parts(self, actor, models):
        ...
        from game.actor_assembly import rot_xyz
        read, mem, a = self.mem.read, self.mem, actor.address
...
class Posed:
    ...
    def __init__(self, name, note, pieces, riders, position, yaw, owners=None):
        from game.actor_assembly import rot_y
        self._rot_y = rot_y
