...
(FUN_A01__801311fc, game/environment_meshes.py) is a real E2 window
...
That is the waterfall formats/animation/uv_anim.py already finds and animates, so
...
at those faces is dropped (formats/animation/clut_animation.py).
...
CELL = 64
# Faces game/environment_meshes.py builds carry this flag - past the
# packet byte, so it never meets a real one.
GENERATED = 0x100
# formats/geometry/mdat.py marks a quad's faces with this, also past the packet byte.
QUAD = 0x200
# formats/models/smst_parser.py marks every face with this: an actor's model part,
