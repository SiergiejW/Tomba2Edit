...
see game/placement.py. That is settled by the routine the record
...
from psx.mips import LOADS, STORES, Image
...
def load_overlay(path, base=None):
    ...
    folder the same way formats/animation/clut_anim.py gets it - every overlay
...
class CodeModels:
    ...
    def bindings(self, placements):
        ...
        table - the same shape game.placement.load_bindings
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(__doc__)
        print("usage: python -m game.handler_models <CD folder> "
              "<BIN folder> <MAIN.EXE>")
        raise SystemExit(2)
    _check(*sys.argv[1:4])
